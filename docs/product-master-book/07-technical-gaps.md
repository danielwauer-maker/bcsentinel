...
5. **GAP-MAIL-001:** SMTP-Sendewege sind implementiert, überspringen Versand bei fehlender Konfiguration; reale Zustellung ist nicht automatisiert verifiziert (`dashboard_invite_service.py`, `partners.py`).
...
10. **GAP-OPS-002:** Backend-Recovery läuft als In-Process-Task und der Docker-Stack enthält keinen separaten Worker-Service; BC-Scheduling läuft getrennt über TaskScheduler (`main.py`, Compose, `DHScanSchedulerMgt`). Mehrinstanzverhalten ist statisch nicht entscheidbar.
