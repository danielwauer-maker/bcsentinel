...
CURRENT_VERSION = "1.0.2.21"
BASELINE_VERSION = "1.0.2.20"
REQUIRED_PLATFORM = "27.0.0.0"
REQUIRED_RUNTIME = "16.0"
BASELINE_APP_FILE = "BCSentinel Analytics - Daniel Wauer_BCSentinel_1.0.2.20.app"
BASELINE_APP_SHA256 = "62a5a5d380008f3d212bbc2834429ad4b3f3d36787b4ee567b7e68a2f4e78756"
...
def test_current_release_targets_bc27_and_version_1_0_2_21():
    app = _read_json(APP_MANIFEST)
    assert app["version"] == CURRENT_VERSION
    assert app["platform"] == REQUIRED_PLATFORM
    assert app["application"] == REQUIRED_PLATFORM
    assert app["runtime"] == REQUIRED_RUNTIME


def test_ext_50_01_release_manifest_remains_verified_historical_evidence():
    app = _read_json(APP_MANIFEST)
    ...
    assert release["extension"]["version"] == BASELINE_VERSION
    ...
    assert release["artifact"]["file_name"] == BASELINE_APP_FILE
    assert release["artifact"]["sha256"] == BASELINE_APP_SHA256
