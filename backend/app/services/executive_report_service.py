...
def build_executive_report(db: Session, tenant: Tenant, scan_id: str) -> ExecutiveReport:
    ...
    return ExecutiveReport(
        ...
        kpis=[
            ...
            ReportKpi(label="Prüftreffer" if language == "de" else "Check occurrences", value=_number(affected_records, language), note=f"Ueber {_safe_int(scan.issues_count)} Findings" if language == "de" else f"Across {_safe_int(scan.issues_count)} findings"),
        ],
...
def _render_executive_report_pdf_fallback(report: ExecutiveReport) -> bytes:
    ...
    lines = [
        ...
        f"{'Check occurrences' if language == 'en' else 'Prüftreffer'}: {_number(report.affected_records, language)}",
